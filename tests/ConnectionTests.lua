...
function testLoginInvalidName()  
  ...
  denyReason, _ = connection:canLogin(2, "L$3t", "1.1.1.1", consts.ENGINE_VERSION)
  assert(denyReason ~= nil)

  -- NAME_LENGTH_LIMIT
  denyReason, _ = connection:canLogin(2, "testtesttesttesttesttest", "1.1.1.1", consts.ENGINE_VERSION)
  assert(denyReason ~= nil)
end
...
function testLoginInvalidUserID()  
  ...
  denyReason, _ = connection:canLogin("need a new user id", "Jerry", "1.1.1.1", consts.ENGINE_VERSION)
  assert(denyReason ~= nil)

  -- fake ID
  local _, playerBan = connection:canLogin(42, "Bob", "1.1.1.1", consts.ENGINE_VERSION)
  assert(playerBan ~= nil)

  -- have account, name taken and doesn't match ID
  denyReason, _ = connection:canLogin(2, "Jerry", "1.1.1.1", consts.ENGINE_VERSION)
  assert(denyReason ~= nil)
end
...
function testLoginAllowed()
  ...
  denyReason, playerBan = connection:canLogin(2, "Jeremy", "1.1.1.1", consts.ENGINE_VERSION)
  assert(denyReason == nil and playerBan == nil)

  -- can login if have account and name isn't changed
  denyReason, playerBan = connection:canLogin(2, "Ben", "1.1.1.1", consts.ENGINE_VERSION)
  assert(denyReason == nil and playerBan == nil)
