...
function Stack.new_row(self)
  ...
  if string.len(self.panel_buffer) <= 10 * self.width then
    ...
    self.panel_buffer = PanelGenerator.makePanels(self.match.seed + self.panelGenCount, self.levelData.colors, self.panel_buffer, self.match.mode, self.level, opponentLevel)
...
function Stack.onPop(self, panel)
  if panel.isGarbage then
    ...
    if self.match.mode == "vs" and self.panels_cleared % self.levelData.shockFrequency == 0 then
