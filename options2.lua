...
local function graphics_menu()
  ...
  local function update_theme()
    config.theme = themes[theme_index]
    graphicsMenu:set_button_setting(1, config.theme)
  end

  local function next_theme()
    theme_index = bound(1, theme_index + 1, #themes)
    update_theme()
  end
...
function options2.main(button_idx)
  local ret = nil
  local menu_x, menu_y = unpack(main_menu_screen_pos)
  menu_y = menu_y + 70
  ...
  local function update_language(update_text)
    ...
    if update_text then
      ret = {options2.main} -- this allows the menu to change the text
    end
  end
