...
local function testComboQueueing1()
  ...
  assert(#stack.inTransitGarbage == 0, "3 wide should delay 4 wide so that both are still in telegraph")
  ...
  assert(tableUtils.length(stack.inTransitGarbage) == 1, "3 and 4 wide should've left telegraph together")
  assert(stack.inTransitGarbage[420 + minTransferTime], "3 wide should've managed to pass through telegraph in the shortest time possible")
  assert(stack.telegraph.garbage_queue:len() == 1, "5 wide should still be inside")
  StackReplayTestingUtils:cleanup(match)
end

-- TODO: readjust for inTransitGarbage moving into the sender's garbage queue
--testComboQueueing1()