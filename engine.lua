...
function Stack:assertEquality(other)
  ...
  -- game stopwatch may have a diff of 1 frame on game end
  assert(math.abs(other.game_stopwatch - self.game_stopwatch) <= 1, "game_stopwatch diff is too big")
...
function Stack.new_row(self)
  ...
  for row = stackHeight, 1, -1 do
    for col = #panels[row], 1, -1 do
      ...
      panels[row][col].rowChanged = true
    end
  end
