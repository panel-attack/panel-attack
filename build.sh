...
zip -r --quiet "${build_dir}panel-attack.love" *.lua *.ttf *.otf readme*.txt *.md *.ogg characters computerPlayers default_data engine helpers libraries localization.csv network panels rich_presence scenes select_screen stages themes ui -x ".*" \*__MACOSX* \*\.DS_Store
