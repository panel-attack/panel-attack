...
local insertPlayerELOChangeStatement = assert(db:prepare("INSERT INTO PlayerELOHistory(publicPlayerID, rating) VALUES (?, ?)"))
function PADatabase.insertPlayerELOChange(publicPlayerID, rating)
  insertPlayerELOChangeStatement:bind_values(publicPlayerID, rating)
  insertPlayerELOChangeStatement:step()
  if insertPlayerELOChangeStatement ~= 0 then
    print(db:errmsg())
  end
end
