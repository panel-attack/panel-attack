require("engine")
require("profiler")
require("computerPlayers.TreeSearchComputer.StackExtensions")
...
TreeSearchComputer = class(function(self)  
    self.name = "TreeSearchComputer"
    self.stack = nil
    self.config = nil
end)

function TreeSearchComputer.initializeConfig(self, config)
...
function TreeSearchComputer.getConfigs()
    return cpuConfigs
end

function TreeSearchComputer.setConfig(self, config)
    if config then
...
function TreeSearchComputer.updateStack(self, stack)
    self.stack = stack
  end
...
function TreeSearchComputer.isMovement(self, input)
...
function TreeSearchComputer.cpuLog(self, level, ...)
  if self.config.log >= level then
      print(...)
  end
end

function TreeSearchComputer.getInput(self)
...
function TreeSearchComputer.allActions(self, stack, actionsAllowed)
  local actions = {}

  --actions[#actions + 1] = waitInput

  if not actionsAllowed or actionsAllowed > 0 then
    if StackExtensions.rowEmpty(stack, STACK_ROW_EMPTY_ALLOWING_RAISE) then
...
function TreeSearchComputer.idleAction(self, idleCount)
  local result = ""
...
function TreeSearchComputer.moveToRowColumnAction(self, row, column)
...
function TreeSearchComputer.addAction(self, stack, action)
  stack.input_buffer = stack.input_buffer .. action
end

function TreeSearchComputer.panelsScore(self)
...
function TreeSearchComputer.heuristicValueForStack(self)
  ...
  if StackExtensions.rowEmpty(self.stack, STACK_LOW_ROW) then
    local panelCount = StackExtensions.panelCount(self.stack)
  ...
  -- for index = 8, 12, 1 do
  --   if StackExtensions.rowEmpty(self.stack, index) == false then
...
function TreeSearchComputer.heuristicValueForIdleFilledStack(self)
  self:fillIdleActions()
  self:simulateTillEqual()
  return self:heuristicValueForStack()
end

function TreeSearchComputer.simulateAllInputs(self)
  self.stack:run(#self.stack.input_buffer)
end

function TreeSearchComputer.simulateTillEqual(self)
...
function TreeSearchComputer.fillIdleActions(self)
...
--[[
function TreeSearchComputer.fillRandomActions(self)
  while self.stack.CLOCK + #self.stack.input_buffer <= self.stack.garbage_target.CLOCK + #self.stack.garbage_target.input_buffer do
...
function TreeSearchComputer.playoutValueForStack(self, maxClock)
...
function TreeSearchComputer.monteCarloValueForStack(self, maxClock, n)
  
  n = n or MONTE_CARLO_RUN_COUNT
  local sum = 0
  for index = 1, n do
    local copiedStack = StackExtensions.copyStack(self.stack)
...
function TreeSearchComputer.bestAction(self, maxClock, depthAllowed)
  ...
  for idx = 1, #actions do
    ...
    local simulatedStack = StackExtensions.copyStack(self.stack)
...
function TreeSearchComputer.yieldIfTooLong(self)
...
function TreeSearchComputer.think(self)
  ...
  while true do
    if self.unexpandedQueue:size() > 0 then
      ...
      if gameResult == nil then
        ...
        for idx = 1, #actions do
          local action = actions[idx]
          local simulatedStack = StackExtensions.copyStack(expand.stack)
          self:addAction(simulatedStack, action)
...
-- function TreeSearchComputer.mctsValue(self) 
--   self = current
...
function TreeSearchComputer.expandValue(self, value, depth)
...
function TreeSearchComputer.initThink(self, idleCount)
  self.resultQueue = PriorityQueue()
  self.unexpandedQueue = PriorityQueue()

  self.stack = StackExtensions.copyStack(self.stack)
...
function TreeSearchComputer.clearThink(self)
  self.resultQueue = PriorityQueue()
  self.unexpandedQueue = PriorityQueue()
end

function TreeSearchComputer.currentResult(self) 
