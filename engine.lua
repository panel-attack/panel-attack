...
Stack =
  ...
  function(s, arguments)
    ...
    s.panel_buffer = ""
    s.gpanel_buffer = ""
    s.input_buffer = List() -- Inputs that haven't been processed yet
    s.confirmedInput = List(true) -- All inputs the player has input ever
    s.panels = {}
    s.width = 6
    s.height = 12
    for i = 0, s.height do
...
function Stack.restoreFromRollbackCopy(self, other)
  ...
  for i = self.CLOCK + 1, confirmedInput:length() do
    self.input_buffer:append(self.confirmedInput[i])
  end
end
...
function Stack.shouldRun(self, runsSoFar) 
  ...
  local buffer_len = self.input_buffer:length()
  ...
  if config.debug_mode and config.debug_vsFramesBehind and config.debug_vsFramesBehind ~= 0 then
    if (config.debug_vsFramesBehind > 0) == (self.which == 2) then
      ...
      if self.garbage_target and self.garbage_target:game_ended() == false then
        ...
        if network_connected() or self.garbage_target.input_buffer:length() > 0 then
...
function Stack.run(self)
  ...
  if self.is_local == false then
    if self.play_to_end then
      ...
      if self.input_buffer:length() < 4 then
...
function Stack.setupInput(self) 
  ...
  if self:game_ended() == false then 
    if self.input_buffer and self.input_buffer:length() > 0 then
...
function Stack.receiveConfirmedInput(self, input)
  if string.len(input) == 1 then
    self.confirmedInput:append(input)
    self.input_buffer:append(input)
  else
    local inputs = string.toCharTable(input)
    self.confirmedInput:appendList(inputs)
    self.input_buffer:appendList(inputs)
  end
  --logger.debug("Player " .. self.which .. " got new input. Total length: " .. confirmedInput:length()
end
